...
public class ConfigureInteractChannel implements Configurable{
    @Override
        ...
        switch(args[2]){
            case "add":{
                try{
                    ...
                    if(server.getConfig().getInteract_channel().contains(channel.getId())){
                    ...
                    server.getConfig().getInteract_channel().forEach(
                            x -> {
                                old.add("#" + message.getGuild().getGuildChannelById(x).getName());
                            }
                    );
                    server.getConfig().getInteract_channel().add(channel.getId());
            ...
            case "delete":{
                try{
                    ...
                    server.getConfig().getInteract_channel().remove(channel.getId());
                    new Database().updateServer(server.getServer_id(), server);
            ...
            case "get":{
                ...
                message.getChannel().sendMessage(String.format("[:robot:] = [%s]", String.join(", ", channelNames))).queue();
                break;
            }
        }
    }
    ...
    public String getUsage() {
        return "Usage: !config interact [add|delete|get] [#channel]";
    }
}
